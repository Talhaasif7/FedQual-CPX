...
class DriftConfig:
    ...
    enabled: bool = False
    drift_type: str = "none"            # 'none', 'class_swap', 'feature_noise', 'label_noise'
    drift_round: int = 50              # Round tau when drift occurs
...
class DriftedDataset(Dataset):
    ...
    def set_active(self, active: bool) -> None:
        """Enable or disable drift transformation."""
...
class DriftManager:
    """Manages piecewise-stationary drift across federated clients.
    ...
    def _init_transforms(self) -> None:
        ...
        for client_id in self.drift_client_ids:
            ...
            if self.drift_type == "class_swap":
            ...
            elif self.drift_type == "feature_noise":
                # D3: Gaussian feature noise
                ...
                def make_feature_noise(std: float, c_rng: np.random.Generator):
                    def _transform(x: torch.Tensor, y: int) -> tuple[torch.Tensor, int]:
                        ...
                        return torch.clamp(x + noise, -3.0, 3.0), y
                    return _transform

                self._transforms[client_id] = make_feature_noise(noise_std, client_rng)

            elif self.drift_type == "label_noise":
    ...
    def wrap_client_dataset(
        ...
        return DriftedDataset(dataset, transform_fn=transform, is_active=active)

    def update_round(self, current_round: int, client_datasets: dict[int, DriftedDataset]) -> None:
        """Update active states for all client datasets according to the current round."""
        for client_id, ds in client_datasets.items():
            ds.set_active(self.is_client_drifted(client_id, current_round))
